"""
幸福之家管理系統 Pro v13.7 - 完整功能修復版
本次更新內容：
1. 【修復】恢復完整的電費管理功能 (包含台電輸入、度數輸入、自動分攤計算)
2. 【保留】儀表板優化 (租約到期倒數、房間狀態卡片)
3. 【保留】收租金優化 (智慧試算面板)
4. 【保留】全域 CSS 美化
"""
    ...
    with tab1:
        ...
        with st.container(border=True):
            ...
            with st.expander("填寫收款詳情 (若已收款)", expanded=True):
                c1, c2 = st.columns(2)
                with c1: paid_amt = st.number_input("實收金額", value=0.0, step=100.0, help="輸入 0 表示尚未收到")
...
def main():
    st.set_page_config(page_title="幸福之家 v13.7", page_icon="🏠", layout="wide")
